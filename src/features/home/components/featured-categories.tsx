      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 min-[480px]:grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3 sm:gap-4.5 lg:gap-6">
          {categoriesList.map((category) => (
            ...
            >
              <div className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center text-muted-foreground group-hover:text-[#F35C30] dark:group-hover:text-[#F35C30] transition-colors">
                {category.imageUrl && (
                  <Image
              ...
              <span className="text-xs sm:text-sm font-medium text-foreground group-hover:text-[#F35C30] dark:group-hover:text-[#F35C30] transition-colors text-center line-clamp-2 mt-2">
                {category.name}
              </span>
            </Link>
          ))}
