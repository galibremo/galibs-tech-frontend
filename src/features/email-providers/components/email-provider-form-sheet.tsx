...
type BooleanFieldName =
  "emailSecure" | "iterableAllowRepeatMarketingSends" | "sparkpostSandbox";

type InputFieldName = Exclude<
  keyof EmailProviderFormValues,
  "providerType" | BooleanFieldName
>;
...
export function EmailProviderFormSheet({
    ...
    switch (values.providerType) {
      case "brevo": {
        const config: Record<string, unknown> = {
          apiKey:
            values.brevoApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.brevoApiKey,
      ...
      case "resend": {
        const config: Record<string, unknown> = {
          apiKey:
            values.resendApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.resendApiKey,
          senderEmail: values.resendSenderEmail,
          senderName: values.resendSenderName,
          ...(omitIfBlank(values.resendBaseUrl) && {
            baseUrl: values.resendBaseUrl.trim(),
          }),
        };
      ...
      case "email": {
        const config: Record<string, unknown> = {
          ...
          auth: {
            ...
            pass:
              values.emailPass === MASKED_VALUE && isEditing
                ? undefined
                : values.emailPass,
          },
      ...
      case "ses": {
        const config: Record<string, unknown> = {
          accessKeyId:
            values.awsAccessKeyId === MASKED_VALUE && isEditing
              ? undefined
          ...
          ...(omitIfBlank(values.awsSessionToken) && {
            sessionToken: values.awsSessionToken.trim(),
          }),
          ...(omitIfBlank(values.awsBaseUrl) && {
            baseUrl: values.awsBaseUrl.trim(),
          }),
        };
      ...
      case "postmark": {
        const config: Record<string, unknown> = {
          ...
          ...(omitIfBlank(values.postmarkBaseUrl) && {
            baseUrl: values.postmarkBaseUrl.trim(),
          }),
          ...(omitIfBlank(values.postmarkMessageStream) && {
            messageStream: values.postmarkMessageStream.trim(),
          }),
        };
        if (isEditing && values.postmarkServerToken === MASKED_VALUE)
      ...
      case "sendgrid": {
        const config: Record<string, unknown> = {
          apiKey:
            values.sendgridApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.sendgridApiKey,
          senderEmail: values.sendgridSenderEmail,
          senderName: values.sendgridSenderName,
          ...(omitIfBlank(values.sendgridBaseUrl) && {
            baseUrl: values.sendgridBaseUrl.trim(),
          }),
        };
        if (isEditing && values.sendgridApiKey === MASKED_VALUE)
          delete config.apiKey;
        return config;
      }
      case "cloudflare": {
        const config: Record<string, unknown> = {
          ...
          ...(omitIfBlank(values.cloudflareBaseUrl) && {
            baseUrl: values.cloudflareBaseUrl.trim(),
          }),
        };
        if (isEditing && values.cloudflareApiToken === MASKED_VALUE)
      ...
      case "unosend": {
        const config: Record<string, unknown> = {
          apiKey:
            values.unosendApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.unosendApiKey,
          senderEmail: values.unosendSenderEmail,
          senderName: values.unosendSenderName,
          ...(omitIfBlank(values.unosendBaseUrl) && {
            baseUrl: values.unosendBaseUrl.trim(),
          }),
        };
        if (isEditing && values.unosendApiKey === MASKED_VALUE)
          delete config.apiKey;
        return config;
      }
      case "iterable": {
        const config: Record<string, unknown> = {
          apiKey:
            values.iterableApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.iterableApiKey,
          ...
          ...(omitIfBlank(values.iterableBaseUrl) && {
            baseUrl: values.iterableBaseUrl.trim(),
          }),
        };
        if (isEditing && values.iterableApiKey === MASKED_VALUE)
          delete config.apiKey;
        return config;
      }
      case "mailgun": {
        const config: Record<string, unknown> = {
          apiKey:
            values.mailgunApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.mailgunApiKey,
          domain: values.mailgunDomain,
          senderEmail: values.mailgunSenderEmail,
          senderName: values.mailgunSenderName,
          ...(omitIfBlank(values.mailgunBaseUrl) && {
            baseUrl: values.mailgunBaseUrl.trim(),
          }),
        };
        if (isEditing && values.mailgunApiKey === MASKED_VALUE)
          delete config.apiKey;
        return config;
      }
      case "mailersend": {
        const config: Record<string, unknown> = {
          ...
          ...(omitIfBlank(values.mailersendBaseUrl) && {
            baseUrl: values.mailersendBaseUrl.trim(),
          }),
        };
        if (isEditing && values.mailersendApiKey === MASKED_VALUE)
          delete config.apiKey;
        return config;
      }
      case "mailchimp": {
        const config: Record<string, unknown> = {
          apiKey:
            values.mailchimpApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.mailchimpApiKey,
          senderEmail: values.mailchimpSenderEmail,
          senderName: values.mailchimpSenderName,
          ...(omitIfBlank(values.mailchimpBaseUrl) && {
            baseUrl: values.mailchimpBaseUrl.trim(),
          }),
        };
        if (isEditing && values.mailchimpApiKey === MASKED_VALUE)
          delete config.apiKey;
        return config;
      }
      case "sparkpost": {
        const config: Record<string, unknown> = {
          apiKey:
            values.sparkpostApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.sparkpostApiKey,
          senderEmail: values.sparkpostSenderEmail,
          senderName: values.sparkpostSenderName,
          sandbox: values.sparkpostSandbox,
          ...(omitIfBlank(values.sparkpostBaseUrl) && {
            baseUrl: values.sparkpostBaseUrl.trim(),
          }),
        };
        if (isEditing && values.sparkpostApiKey === MASKED_VALUE)
          delete config.apiKey;
        return config;
      }
      case "loops": {
        const config: Record<string, unknown> = {
          apiKey:
            values.loopsApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.loopsApiKey,
          transactionalId: values.loopsTransactionalId,
          senderEmail: values.loopsSenderEmail,
          senderName: values.loopsSenderName,
          ...(omitIfBlank(values.loopsBaseUrl) && {
            baseUrl: values.loopsBaseUrl.trim(),
          }),
        };
        if (isEditing && values.loopsApiKey === MASKED_VALUE)
          delete config.apiKey;
        return config;
      }
      case "sequenzy": {
        const config: Record<string, unknown> = {
          apiKey:
            values.sequenzyApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.sequenzyApiKey,
          senderEmail: values.sequenzySenderEmail,
          senderName: values.sequenzySenderName,
          ...(omitIfBlank(values.sequenzyBaseUrl) && {
            baseUrl: values.sequenzyBaseUrl.trim(),
          }),
        };
        if (isEditing && values.sequenzyApiKey === MASKED_VALUE)
          delete config.apiKey;
        return config;
      }
      case "jetemail": {
        const config: Record<string, unknown> = {
          apiKey:
            values.jetemailApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.jetemailApiKey,
          senderEmail: values.jetemailSenderEmail,
          senderName: values.jetemailSenderName,
          ...(omitIfBlank(values.jetemailBaseUrl) && {
            baseUrl: values.jetemailBaseUrl.trim(),
          }),
        };
        if (isEditing && values.jetemailApiKey === MASKED_VALUE)
          delete config.apiKey;
        return config;
      }
      case "lettermint": {
        const config: Record<string, unknown> = {
          ...
          ...(omitIfBlank(values.lettermintBaseUrl) && {
            baseUrl: values.lettermintBaseUrl.trim(),
          }),
          ...(omitIfBlank(values.lettermintRoute) && {
            route: values.lettermintRoute.trim(),
          }),
        };
        if (isEditing && values.lettermintApiToken === MASKED_VALUE)
          delete config.apiToken;
        return config;
      }
      case "primitive": {
        const config: Record<string, unknown> = {
          apiKey:
            values.primitiveApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.primitiveApiKey,
          senderEmail: values.primitiveSenderEmail,
          senderName: values.primitiveSenderName,
          ...(omitIfBlank(values.primitiveBaseUrl) && {
            baseUrl: values.primitiveBaseUrl.trim(),
          }),
        };
        if (isEditing && values.primitiveApiKey === MASKED_VALUE)
          delete config.apiKey;
        return config;
      }
      case "plunk": {
        const config: Record<string, unknown> = {
          apiKey:
            values.plunkApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.plunkApiKey,
          senderEmail: values.plunkSenderEmail,
          senderName: values.plunkSenderName,
          ...(omitIfBlank(values.plunkBaseUrl) && {
            baseUrl: values.plunkBaseUrl.trim(),
          }),
        };
        if (isEditing && values.plunkApiKey === MASKED_VALUE)
          delete config.apiKey;
        return config;
      }
      case "mailtrap": {
        const config: Record<string, unknown> = {
          apiKey:
            values.mailtrapApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.mailtrapApiKey,
          senderEmail: values.mailtrapSenderEmail,
          senderName: values.mailtrapSenderName,
          ...(omitIfBlank(values.mailtrapBaseUrl) && {
            baseUrl: values.mailtrapBaseUrl.trim(),
          }),
        };
        if (isEditing && values.mailtrapApiKey === MASKED_VALUE)
          delete config.apiKey;
        return config;
      }
      case "scaleway": {
        const config: Record<string, unknown> = {
          ...
          ...(omitIfBlank(values.scalewayRegion) && {
            region: values.scalewayRegion.trim(),
          }),
          ...(omitIfBlank(values.scalewayBaseUrl) && {
            baseUrl: values.scalewayBaseUrl.trim(),
          }),
        };
        if (isEditing && values.scalewaySecretKey === MASKED_VALUE)
      ...
      case "zeptomail": {
        const config: Record<string, unknown> = {
          token:
            values.zeptomailToken === MASKED_VALUE && isEditing
              ? undefined
              : values.zeptomailToken,
          senderEmail: values.zeptomailSenderEmail,
          senderName: values.zeptomailSenderName,
          ...(omitIfBlank(values.zeptomailBaseUrl) && {
            baseUrl: values.zeptomailBaseUrl.trim(),
          }),
        };
        if (isEditing && values.zeptomailToken === MASKED_VALUE)
          delete config.token;
        return config;
      }
      case "mailpace": {
        const config: Record<string, unknown> = {
          apiKey:
            values.mailpaceApiKey === MASKED_VALUE && isEditing
              ? undefined
              : values.mailpaceApiKey,
          senderEmail: values.mailpaceSenderEmail,
          senderName: values.mailpaceSenderName,
          ...(omitIfBlank(values.mailpaceBaseUrl) && {
            baseUrl: values.mailpaceBaseUrl.trim(),
          }),
        };
        if (isEditing && values.mailpaceApiKey === MASKED_VALUE)
          delete config.apiKey;
        return config;
      }
    }
  ...
  const onSubmit = async (values: EmailProviderFormValues) => {
    ...
    try {
      ...
      const message =
        error instanceof ApiError ? error.message : "Failed to save provider";
      toast.error(message);
    }
  };
      ...
      >
        <FormProvider {...form}>
          ...
          >
            <SheetHeader>
              <SheetTitle>
                {isEditing ? "Edit Provider" : "Add Email Provider"}
              </SheetTitle>
              <SheetDescription>
                {isEditing
...
function EmailProviderFormFields({
  ...
  return (
    <FieldGroup className="gap-4">
      ...
      {!isEditing ? (
        <Field>
          <FieldLabel htmlFor={`${idPrefix}-provider-type`}>
            Provider Type
          </FieldLabel>
          <Controller
            name="providerType"
            ...
            render={({ field }) => (
              <Select
                value={field.value}
                onValueChange={(value) =>
                  field.onChange(value as EmailProviderType)
                }
                disabled={disabled}
              >
                <SelectTrigger
      ...
      {providerType === "email" ? (
        <>
          ...
          >
            <div>
              <FieldLabel htmlFor={`${idPrefix}-email-secure`}>
                Use SSL/TLS
              </FieldLabel>
              <FieldDescription>
                Enable secure Email connections.
              </FieldDescription>
            </div>
            <Controller
              name="emailSecure"
      ...
      {providerType === "sparkpost" ? (
        <>
          ...
          >
            <div>
              <FieldLabel htmlFor={`${idPrefix}-sparkpost-sandbox`}>
                Sandbox Mode
              </FieldLabel>
              <FieldDescription>
...
function getDefaultValues(
  ...
  if (provider) {
    ...
    return {
      ...
      emailSenderEmail:
        (config as unknown as EmailServerConfig)?.senderEmail ?? "",
      emailSenderName:
        (config as unknown as EmailServerConfig)?.senderName ?? "",
      // ses
      ...
      postmarkSenderEmail:
        (config as unknown as PostmarkConfig)?.senderEmail ?? "",
      postmarkSenderName:
        (config as unknown as PostmarkConfig)?.senderName ?? "",
      postmarkBaseUrl: (config as unknown as PostmarkConfig)?.baseUrl ?? "",
      postmarkMessageStream:
        (config as unknown as PostmarkConfig)?.messageStream ?? "",
      // sendgrid
      sendgridApiKey: MASKED_VALUE,
      sendgridSenderEmail:
        (config as unknown as SendgridConfig)?.senderEmail ?? "",
      sendgridSenderName:
        (config as unknown as SendgridConfig)?.senderName ?? "",
      sendgridBaseUrl: (config as unknown as SendgridConfig)?.baseUrl ?? "",
      // cloudflare
      cloudflareApiToken: MASKED_VALUE,
      cloudflareAccountId:
        (config as unknown as CloudflareConfig)?.accountId ?? "",
      cloudflareSenderEmail:
        (config as unknown as CloudflareConfig)?.senderEmail ?? "",
      cloudflareSenderName:
        (config as unknown as CloudflareConfig)?.senderName ?? "",
      cloudflareBaseUrl: (config as unknown as CloudflareConfig)?.baseUrl ?? "",
      // unosend
      unosendApiKey: MASKED_VALUE,
      unosendSenderEmail:
        (config as unknown as UnosendConfig)?.senderEmail ?? "",
      ...
      iterableCampaignId:
        (config as unknown as IterableConfig)?.campaignId ?? Number.NaN,
      iterableSenderEmail:
        (config as unknown as IterableConfig)?.senderEmail ?? "",
      iterableSenderName:
        (config as unknown as IterableConfig)?.senderName ?? "",
      iterableBaseUrl: (config as unknown as IterableConfig)?.baseUrl ?? "",
      iterableAllowRepeatMarketingSends:
        (config as unknown as IterableConfig)?.allowRepeatMarketingSends ??
        false,
      // mailgun
      mailgunApiKey: MASKED_VALUE,
      mailgunDomain: (config as unknown as MailgunConfig)?.domain ?? "",
      mailgunSenderEmail:
        (config as unknown as MailgunConfig)?.senderEmail ?? "",
      ...
      mailersendSenderEmail:
        (config as unknown as MailersendConfig)?.senderEmail ?? "",
      mailersendSenderName:
        (config as unknown as MailersendConfig)?.senderName ?? "",
      mailersendBaseUrl: (config as unknown as MailersendConfig)?.baseUrl ?? "",
      // mailchimp
      mailchimpApiKey: MASKED_VALUE,
      mailchimpSenderEmail:
        (config as unknown as MailchimpConfig)?.senderEmail ?? "",
      mailchimpSenderName:
        (config as unknown as MailchimpConfig)?.senderName ?? "",
      mailchimpBaseUrl: (config as unknown as MailchimpConfig)?.baseUrl ?? "",
      // sparkpost
      sparkpostApiKey: MASKED_VALUE,
      sparkpostSenderEmail:
        (config as unknown as SparkpostConfig)?.senderEmail ?? "",
      sparkpostSenderName:
        (config as unknown as SparkpostConfig)?.senderName ?? "",
      sparkpostBaseUrl: (config as unknown as SparkpostConfig)?.baseUrl ?? "",
      sparkpostSandbox:
        (config as unknown as SparkpostConfig)?.sandbox ?? false,
      // loops
      loopsApiKey: MASKED_VALUE,
      loopsTransactionalId:
        (config as unknown as LoopsConfig)?.transactionalId ?? "",
      ...
      sequenzySenderEmail:
        (config as unknown as SequenzyConfig)?.senderEmail ?? "",
      sequenzySenderName:
        (config as unknown as SequenzyConfig)?.senderName ?? "",
      sequenzyBaseUrl: (config as unknown as SequenzyConfig)?.baseUrl ?? "",
      // jetemail
      jetemailApiKey: MASKED_VALUE,
      jetemailSenderEmail:
        (config as unknown as JetemailConfig)?.senderEmail ?? "",
      jetemailSenderName:
        (config as unknown as JetemailConfig)?.senderName ?? "",
      jetemailBaseUrl: (config as unknown as JetemailConfig)?.baseUrl ?? "",
      // lettermint
      lettermintApiToken: MASKED_VALUE,
      lettermintSenderEmail:
        (config as unknown as LettermintConfig)?.senderEmail ?? "",
      lettermintSenderName:
        (config as unknown as LettermintConfig)?.senderName ?? "",
      ...
      primitiveSenderEmail:
        (config as unknown as PrimitiveConfig)?.senderEmail ?? "",
      primitiveSenderName:
        (config as unknown as PrimitiveConfig)?.senderName ?? "",
      ...
      mailtrapSenderEmail:
        (config as unknown as MailtrapConfig)?.senderEmail ?? "",
      mailtrapSenderName:
        (config as unknown as MailtrapConfig)?.senderName ?? "",
      ...
      scalewaySenderEmail:
        (config as unknown as ScalewayConfig)?.senderEmail ?? "",
      scalewaySenderName:
        (config as unknown as ScalewayConfig)?.senderName ?? "",
      ...
      zeptomailSenderEmail:
        (config as unknown as ZeptomailConfig)?.senderEmail ?? "",
      zeptomailSenderName:
        (config as unknown as ZeptomailConfig)?.senderName ?? "",
      zeptomailBaseUrl: (config as unknown as ZeptomailConfig)?.baseUrl ?? "",
      // mailpace
      mailpaceApiKey: MASKED_VALUE,
      mailpaceSenderEmail:
        (config as unknown as MailpaceConfig)?.senderEmail ?? "",
      mailpaceSenderName:
        (config as unknown as MailpaceConfig)?.senderName ?? "",
